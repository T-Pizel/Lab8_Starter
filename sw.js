...
self.addEventListener('install', function (event) {
  event.waitUntil(
    caches.open(CACHE_NAME).then(function (cache) {
      ...
      return cache.addAll(['/', '/index.html', '/assets/scripts/main.js', '/assets/scripts/RecipeCard.js', 'assets/styles/main.css', RECIPE_URLS]);
    })
  );
});
...
self.addEventListener('fetch', function (event) {
  ...
  event.respondWith(
    caches.open(CACHE_NAME).then(function(cache){
      ...
      // network response.
      return cache.match(event.request).then(function(data) {
        if(data){
          return data;
        }
